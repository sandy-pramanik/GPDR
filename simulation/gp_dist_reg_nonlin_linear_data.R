...
parameters {
  ...
  //real<lower=1,upper=1> sigma; // Spatial Noise level
...
y_true=rep(0,N)
for(i in 1:N){
  y_true[i] <- rnorm(1, mean=mean(sin(2 * pi * xmat[i,])), sd = 0.1)
  }
...
ggplot() +
  ...
  geom_ribbon(aes(x = x_pred, ymin = apply(f_pred_nl, 2, quantile, 0.025), 
              ...
              fill = "red", alpha = 0.2) +
  labs(title = "GP Distribution Regression (non-linear): Coefficent funtion",x = "x",y = "f(x)") +
...
# Plot the true and fitted functions
ggplot() +
  geom_point(aes(t_pred,  t_pred , color = 'true phi'), size = 3, alpha = 0.7) +
  #geom_point(aes(x,  y_true ), color = "green", size = 3, alpha = 0.7) +
  ...
  labs(title = "GP Distribution Regression  (non-linear): Link function",x = "t",y = "phi(t)") +
  scale_color_manual(name='',
...
ggplot() +
  ...
  #geom_point(aes(x,  y_true ), color = "green", size = 3, alpha = 0.7) +
  geom_line(aes(x_pred, apply(f_pred,2,median), color = 'fitted f'), size = 1) +
