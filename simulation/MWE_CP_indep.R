...
set.seed(1)
data = generate_data_cp_indep(beta_gen = beta1, n = 100, m = 100, sig=0.1)
...
### X
set.seed(2)
Xid = sort(sample(x = 1:length(data$X), size = 15, replace = F))
par(mfrow=c(3,5))
# histogram
for(i in Xid) hist(data$X[[i]], freq = F, xlim=c(0,1),#, breaks = 20, ylim = c(0,4.5)
                   main = paste0('i=', i))
# acf
for(i in Xid) acf(data$X[[i]], main = paste0('i=', i))
par(mfrow=c(1,1)) # 12 X 8
...
# specifics for KDE ----
set.seed(0)
data = generate_data_cp_indep(beta_gen = beta1, n = 100, m = 100)
...
# simulate one data using independent continuous covariates and fit GPDR and KDE ----
## simulate
set.seed(1)
data = generate_data_cp_indep(beta_gen = beta1, n = n, m = m, sig=0.1)
...
fig = 
  ...
  ggplot2::labs(
    ...
    # title = paste0('m=', m), 
    # subtitle = paste0('Uncorrelated X (', 'n=', n, ')'),
    x = 'x', y = 'Regression Function'
  )   # 8 X 6
...
# fig.m10 = fig
# fig.m100 = fig
# ggpubr::ggarrange(fig.m10, fig.m100,
#                   nrow = 1, ncol = 2, legend = 'bottom') # 6.5 X 13
# 
# fig.m1000 = fig
# ggpubr::ggarrange(fig.m10, fig.m100, fig.m1000,
#                   nrow = 1, ncol = 3, legend = 'bottom') # 8 X 20
